...
upstream_path: jupyter-book/introduction/rapids_singlecell.ipynb
...
These are direct one-to-one ports of the scanpy functions covered in [Normalization](https://github.com/theislab/single-cell-best-practices/blob/735f26fd270b3beceb4ba79f4a556c912192fe83/jupyter-book/preprocessing_visualization/normalization.ipynb) and [Feature selection](https://github.com/theislab/single-cell-best-practices/blob/735f26fd270b3beceb4ba79f4a556c912192fe83/jupyter-book/preprocessing_visualization/feature_selection.ipynb).
...
- `rsc.dcg.*` — [decoupler](https://decoupler-py.readthedocs.io/) functional analysis (`ulm`, `mlm`, `aucell`, `waggr`, `zscore`). On a million-cell atlas, transcription factor activity inference goes from hours on CPU to minutes on a single GPU. The CPU and GPU APIs are identical aside from the namespace; see the [pathway and gene-set analysis chapter](https://github.com/theislab/single-cell-best-practices/blob/735f26fd270b3beceb4ba79f4a556c912192fe83/jupyter-book/conditions/gsea_pathway.ipynb).
- `rsc.gr.*` — [squidpy](https://squidpy.readthedocs.io/) spatial analysis (`spatial_autocorr`, `co_occurrence`, `ligrec`). Moran's/Geary's spatial autocorrelation and the ligand–receptor permutation test are the routines that benefit most from the GPU; see the [spatial neighborhood chapter](https://github.com/theislab/single-cell-best-practices/blob/735f26fd270b3beceb4ba79f4a556c912192fe83/jupyter-book/spatial/neighborhood.ipynb) for what these do conceptually. The neighborhood graph itself is still built with `sq.gr.spatial_neighbors` on CPU.
- `rsc.ptg.*` — [pertpy](https://pertpy.readthedocs.io/) perturbation distances via the `Distance` class. Only `edistance` is ported today, with more metrics coming soon. The GPU implementation recomputes distances on the fly rather than materializing an n×n cell-distance matrix, so bootstrap variance estimation becomes practical on screens with hundreds of perturbations; see the [perturbation modeling chapter](https://github.com/theislab/single-cell-best-practices/blob/735f26fd270b3beceb4ba79f4a556c912192fe83/jupyter-book/conditions/perturbation_modeling.ipynb).
