...
本章是 v2 第一轮样章的收束章。它不要求药学本科生完整复现 SCBP、OSCA 或 OSTA workflow，也不要求学生掌握所有单细胞算法。它要求学生能识别图形输入、观测单位、视觉编码、参数敏感性和结论边界，并把这些理解转成综合项目交付包：README、data_sources、ai_use_statement、图表证据边界和 6-8 页项目 storyboard。
...
本章主要来自 Week 16、Week 17 和 Week 18。SCBP、OSCA 和 OSTA 是重要背景来源，帮助课程覆盖单细胞和空间组学的现代数据结构、QC、降维、聚类、注释、空间坐标和多模态趋势。但这些素材进入本科正文时必须降级：只训练图形判读、参数意识和证据边界，不要求完整 workflow。
...
评价时不要求学生给出真实细胞注释或空间机制，而要求学生证明自己知道何时停止。一个优秀项目会把 UMAP、marker、空间图和项目结论都写成层级化证据：哪些是图上可观察，哪些是候选解释，哪些需要文献或数据库核验，哪些超出课程范围。

## 证据表达与学习路径

第 12 章承认现代组学的重要性，但不把本科课程升级成完整生信 workflow。SCBP、OSCA 和 OSTA 可以提供结构、术语和图形来源，却不能把学生任务变成复现 notebook。本章的学习任务应始终落在“看懂结构、图形、参数和证据边界”。

单细胞和空间组学在本章中是综合项目的高级图形素材，而不是单独技术终点。项目可以选择一张 UMAP、一张 marker 图或一张空间示意图作为练习对象，但必须说明它是教学模拟、课程化改写或公开素材，并写清输入、图形元素、参数、候选解释和待核验点。

第 2 章的可复现规范在这里重新出现。单细胞图形越复杂，越需要 README、data_sources、ai_use_statement 和 storyboard。没有这些文件，学生很容易只展示一张炫目的图，却说不清图从哪里来、颜色代表什么、AI 改了哪些句子、哪些结论还没有来源支持。

学习时要避免两种极端。第一种是过度简化，把单细胞图只说成“点越近越相似”；第二种是过度展开，把 QC、标准化、邻域图、聚类、轨迹、速度、空间域、多模态全部当作必学流程。更稳妥的做法是选择少数图形，围绕观测单位、视觉编码、参数敏感性和结论边界展开。

## 证据边界

本章不要求完整复现高级 workflow。若学习任务变成运行完整 SCBP、OSCA 或 OSTA 分析，就已经超出本科课程范围。合格目标是读懂图形和证据，而不是复制专业分析流程。

细胞类型和空间区域语言必须降级。任何“这个 cluster 是某细胞类型”“这个 spatial domain 代表某病理区域”的句子，都必须有候选、需要核验或来源支持。AI 生成的 marker 解释尤其要小心，不能因为常识上看似合理就进入结论。

综合项目交付必须具体。README、data_sources、ai_use_statement 和 6-8 页 storyboard 是学习证据，不是可选建议。项目 storyboard 至少应包含问题页、数据来源页、图表页、证据边界页、AI 审计页和待核验页。

第 12 章还要完成全书收束。它回扣第 1 章的证据链、第 2 章的可复现规范、第 4 章的数据质量、第 8 章的图表边界和第 11 章的候选解释降级。若第 12 章只剩单细胞专题，就不能完成综合项目训练。

## 学习自查

可以把本章当作全书闭环自查。一个合格项目应同时包含前沿图形、证据边界和项目交付三件事。若内容只像单细胞简介，综合项目没有收束；若内容只像项目 rubric，现代组学素材没有被充分课程化。

可以用一张 UMAP 或空间示意图自查：点或 spot 代表什么，颜色代表什么，参数可能如何影响图形，哪些解释需要 marker、metadata、数据库或文献核验。若这四个问题无法回答，图形还不能进入项目结论。

项目交付也要自查。README、data_sources、ai_use_statement 和 6-8 页 storyboard 是必需证据，而不是扩展作业。缺少其中任何一项，前沿图形都难以被复核。

最后做边界压力测试。看到“证明”“确认”“导致”“靶点”“机制成立”等强结论词时，先检查是否有明确来源和核验状态。没有来源的句子应降级为候选、提示或待核验。

综合项目要形成闭环。图形解读表应进入 storyboard，storyboard 应回到 README 和 AI 使用声明。如果这几个文件彼此断开，说明只是完成了多个任务；如果它们共同说明同一个问题、同一组数据和同一条证据边界，项目才形成闭环。

还要识别“图形参数也是证据的一部分”。过滤阈值、PCA 维度、邻居数和 resolution 不只是软件设置，它们会影响图形形态和解释。若项目只展示最终 UMAP 或空间图，却没有记录这些参数，结论就缺少复现基础。

本章结束时，应能把一张前沿图形转成项目页。该页至少包含图形来源、观测单位、视觉编码、关键参数、图上观察、候选解释、AI 修改记录和待核验点。若只能说“这张图显示细胞分群明显”，而不能说明点、颜色、参数和来源，说明还停留在看图印象层。

项目 storyboard 也应保持证据顺序。先写问题和数据来源，再写图形和方法，随后写 AI 审计和待核验点，最后才写结论边界。若先展示结论，再回头补来源和参数，读者很难判断图形是否支持该结论。综合项目的严谨性来自这种顺序，而不是来自图形复杂度。

项目包中的每张图都应能回到一个数据来源和一个人工核验点。缺少任一项，都不应进入最终结论。

若图形来自公开素材，还应记录来源和课程化改写范围，避免把示例图误写成自己的分析结果。
...
- 综合项目正式发布前应检查来源列表、AI 使用声明和图表边界是否完整。
