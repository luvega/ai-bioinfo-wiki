...
课程主大纲（doc/课程教学讲稿*.md）   ←—— 36 周教学骨架
        ...
        │       └── GitHub
        │
        └── PDF 书库（pdf_originals / sources/PDF_Library）
...
→ [wiki/index.md](index.md)（全表）

### ⏱️ 我想看 wiki 最近发生了什么

→ [wiki/log.md](log.md)（时间线）
...
- 永远别动 `pdf_originals/` `raw/` `sources/` `doc/`。
...
- 已建页面（首次 bootstrap）：约 25–30 页
- 下一步建议：
  1. 把 36 课时拆成 18 个 `topics/week_xx.md`（目前只起了第 1、2 周）。
  2. 把 AIDD 71 节按主题继续展开到 concept 页（特别是 microarray、scRNA-seq、proteomics）。
  3. 给 ISLP/ISLR 各章建一份独立的 source-chapter 页（目前只有书级摘要）。
