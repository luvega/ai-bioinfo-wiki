...
**整个 `E:\Codex_Projects\AI_Course\` 是 vault 根**。这样既能浏览 `knowledge/`，也能跳回 `course/syllabus/` 与 `raw/` 看原文。

> 不要把 vault 设成旧的 `F:\AI_Course\wiki\` 或新根目录下的 `knowledge\` 子目录——这会让指向 `../course/syllabus/` 等课程文件的链接失效。

## 首次打开步骤

1. Obsidian → **Open another vault** → **Open folder as vault** → 选 `E:\Codex_Projects\AI_Course\`。
2. 因为已有 `.obsidian/`，Obsidian 会**直接使用现有配置**，不会重置。
3. 首次启动会扫描索引，约 10-30 秒（看机器）。
4. 打开 [knowledge/overview.md](../overview.md) 作为入口。
...
| 文件夹 | 颜色 | 用途 |
|---|---|---|
| `knowledge/sources/` | 蓝 | 原始素材摘要 |
| `knowledge/entities/` | 橙 | 实体（库/工具） |
| `knowledge/concepts/` | 绿 | 抽象概念 |
| `knowledge/topics/` | 金 | 教学单元 |
| `knowledge/synthesis/` | 紫 | 综合页 |
...
### Q: 想全文搜索 materials/raw/aidd_bioinformatics 字幕怎么办？
