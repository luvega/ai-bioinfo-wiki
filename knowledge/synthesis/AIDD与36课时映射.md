...
| 11 | 科研图表规范与 SCI 图表表达 | Ch.9 §7 + Ch.10 表达结果图 | ggplot2、火山图、热图、图注与导出规范案例 |
| 12 | 高维数据与数学直觉 | Ch.7 count matrix + Ch.9 metadata | 用表达矩阵解释标准化、距离和高维直觉 |
| 13 | PCA、聚类与热图 | Ch.9 §7 + ISLP Ch.12 | PCA / 聚类 / heatmap 的表达矩阵案例 |
| 14 | 转录组数据分析基础 | Ch.7 全章 | RNA-seq 流程、FASTQ/QC/align/count 只讲链条，不要求学生跑全流程 |
| **15** | **差异表达 + DESeq2 + [富集分析](../concepts/富集分析_GO_KEGG.md)** | **Ch.9 §4-6 + Ch.10 GEO2R** | AIDD 提供 DESeq2/GEO2R，**富集分析需外部素材补**（AIDD 未涉及） |
| 16 | 单细胞转录组可视化 | Ch.9 §8, §9 | scRNA-seq 概念、降维、轨迹和 marker 展示 |
...
| `09_R_for_Bioinformatics/04_Differential_Gene_Expression_..._DESeq2_Preparing_Data.txt` | Week 15 | DESeq2 输入数据 |
| `09_R_for_Bioinformatics/05_Deseq2_Code_Understanding.txt` | Week 15 | DESeq2 代码逐行讲 |
| `09_R_for_Bioinformatics/07_Visualizing_..._ggplot2.txt` | Week 7 / Week 11 | ggplot2 入门 / volcano |
| `09_R_for_Bioinformatics/08_Introduction_to_Single-Cell_RNA_Sequencing.txt` | Week 16 | scRNA-seq 概览 |
| `10_Microarray_..._on_R/02_Introduction_of_Microarray.txt` | Week 15 | microarray 与差异分析背景 |
| `10_..._/05_Microarray_Analysis_Using_GEO2R.txt` | Week 15 | GEO2R 与差异分析思想对照 |

更多映射在后续 ingest 中补充到本表。

## 注意

- **AIDD 字幕需要术语校正**，见 [TERMS.md](../../materials/raw/aidd_bioinformatics/TERMS.md)。
- 引用 AIDD 字幕段落作为讲稿时，**先在 [course/syllabus/AIDD_Bioinformatics_课程结构与内容整理.md] 里找节摘要**，
  再回到 txt 校对。
