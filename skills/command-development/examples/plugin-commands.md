...
Validate environment argument: $1 (must be dev, staging, or prod)
...
Determine check level based on environment $1 (FULL for prod, BASIC otherwise)
...
```markdown
Validate input: $1 (must match expected pattern)
If invalid, explain requirements and show usage.
```
...
3. **Not validating inputs:**
   ...
   Validate $1 is one of: dev, staging, prod
