...
Target: E:/AI_Course/raw/AIDD_Bioinformatics/<chapter>/<lesson>.txt
...
OUTPUT_ROOT = Path("E:/AI_Course/raw/AIDD_Bioinformatics")
...
def write_index(chapter_buckets: dict) -> None:
    ...
    lines.append("")
    lines.append(
        "本目录由 `scripts/clean_aidd_subtitles.py` 自动生成。每个文件均为字幕"
...
def write_terms_dictionary() -> None:
    ...
    lines.append("")
    lines.append("术语规则在 `scripts/clean_aidd_subtitles.py` 中维护，更新规则后请重新运行脚本以再生成全部讲稿。")
